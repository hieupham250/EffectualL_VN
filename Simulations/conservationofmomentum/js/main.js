...
function initBasicCollision() {
  ...
  function draw() {
    ...
    // Ball 1 momentum
    ctx.fillText(
      `p₁ = ${ball1.mass} × ${ball1.velocity.toFixed(
        1
      )} = ${ball1.momentum.toFixed(1)} kg·m/s`,
      20,
      30
    );
    ...
    // Ball 2 momentum
    ctx.fillText(
      `p₂ = ${ball2.mass} × ${ball2.velocity.toFixed(
        1
      )} = ${ball2.momentum.toFixed(1)} kg·m/s`,
      20,
      50
    );
    ...
    ctx.fillText(
      `Initial total momentum = ${initialTotalMomentum.toFixed(1)} kg·m/s`,
      20,
      100
    );
...
function initElasticCollision() {
  ...
  function draw() {
    ...
    ctx.fillText(
      `Tổng động lượng = ${totalMomentum.toFixed(1)} kg·m/s`,
      20,
      80
    );
    ctx.fillText(`Tổng năng lượng = ${totalEnergy.toFixed(1)} J`, 20, 100);

    // Initial values
    ctx.fillText(
      `Động lượng ban đầu = ${initialTotalMomentum.toFixed(1)} kg·m/s`,
      20,
      130
    );
    ctx.fillText(
      `Năng lượng ban đầu = ${initialTotalEnergy.toFixed(1)} J`,
      20,
      150
    );
    ...
    if (Math.abs(totalMomentum - initialTotalMomentum) < 0.1) {
      ctx.fillStyle = "green";
      ctx.fillText("✓ Động lượng được bảo toàn!", width - 200, 80);
    }

    if (Math.abs(totalEnergy - initialTotalEnergy) < 0.1) {
      ctx.fillStyle = "green";
      ctx.fillText("✓ Năng lượng được bảo toàn!", width - 200, 100);
    }
  }
...
function initInelasticCollision() {
  ...
  function updateValues() {
    ...
    object1.kineticEnergy =
      0.5 * object1.mass * object1.velocity * object1.velocity;
    object2.kineticEnergy =
      0.5 * object2.mass * object2.velocity * object2.velocity;
  ...
  function handleCollision() {
    ...
    combinedObject.kineticEnergy =
      0.5 *
      combinedObject.mass *
      combinedObject.velocity *
      combinedObject.velocity;
  }
  ...
  function draw() {
    ...
    if (!hasCollided) {
      ...
      ctx.fillText(
        `p₂ = ${object2.momentum.toFixed(1)} kg·m/s`,
        width - 200,
        30
      );
      ctx.fillText(
        `KE₂ = ${object2.kineticEnergy.toFixed(1)} J`,
        width - 200,
        50
      );
      ...
      ctx.fillText(
        `Total momentum = ${totalMomentum.toFixed(1)} kg·m/s`,
        20,
        80
      );
      ...
      ctx.fillRect(
        combinedObject.x,
        combinedObject.y,
        combinedObject.width,
        combinedObject.height
      );

      // Draw velocity vector
      drawVector(
        combinedObject.x + combinedObject.width / 2,
        combinedObject.y - 20,
        combinedObject.velocity * 5,
        "purple",
        "v"
      );
      ...
      // Initial values
      ctx.fillText(
        `Initial momentum = ${initialTotalMomentum.toFixed(1)} kg·m/s`,
        20,
        80
      );
      ctx.fillText(
        `Initial energy = ${initialTotalEnergy.toFixed(1)} J`,
        20,
        100
      );
...
function initExplosion() {
  ...
  function draw() {
    ...
    if (!hasExploded) {
      ...
      ctx.fillText(
        `Total momentum X = ${totalMomentumX.toFixed(1)} kg·m/s`,
        20,
        30
      );
      ctx.fillText(
        `Total momentum Y = ${totalMomentumY.toFixed(1)} kg·m/s`,
        20,
        50
      );
      ctx.fillText(
        `Total momentum magnitude = ${Math.sqrt(
...
function initRealWorld() {
  ...
  function animateBilliards() {
    ...
    if (!scenarioObjects.billiards.hasCollided) {
      ...
      if (distance <= cueBall.radius + targetBall.radius) {
        ...
        const newVx1 =
          ((cueBall.mass - targetBall.mass) * vx1 + 2 * targetBall.mass * vx2) /
          (cueBall.mass + targetBall.mass);
        const newVx2 =
          ((targetBall.mass - cueBall.mass) * vx2 + 2 * cueBall.mass * vx1) /
          (cueBall.mass + targetBall.mass);
  ...
  function animateRocket() {
    ...
    if (animationTime > 1 && rocket.y > 0) {
      ...
      rocket.velocity.y -=
        scenarioObjects.rocket.thrustForce * 0.1 * animationSpeed;
    }
  ...
  function animateCarCrash() {
    ...
    // Check for collision
    if (
      !scenarioObjects.carCrash.hasCollided &&
      car1.x + car1.width >= car2.x
    ) {
      // Handle collision (inelastic)
      const totalMass = car1.mass + car2.mass;
      const totalMomentum =
        car1.mass * car1.velocity.x + car2.mass * car2.velocity.x;
      ...
      const v1 =
        (car1.mass * car1.velocity.x +
          car2.mass * (2 * car2.velocity.x - elasticity * car1.velocity.x)) /
        totalMass;
      const v2 =
        (car2.mass * car2.velocity.x +
          car1.mass * (2 * car1.velocity.x - elasticity * car2.velocity.x)) /
        totalMass;
  ...
  function drawBilliards() {
    ...
    drawVector(
      cueBall.x,
      cueBall.y,
      cueBall.velocity.x * 5,
      cueBall.velocity.y * 5,
      "#3498db"
    );
    drawVector(
      targetBall.x,
      targetBall.y,
      targetBall.velocity.x * 5,
      targetBall.velocity.y * 5,
      "#e74c3c"
    );
    ...
    ctx.fillText(
      `p₁ = (${p1.x.toFixed(1)}, ${p1.y.toFixed(1)}) kg·m/s`,
      20,
      30
    );
    ctx.fillText(
      `p₂ = (${p2.x.toFixed(1)}, ${p2.y.toFixed(1)}) kg·m/s`,
      20,
      50
    );
    ctx.fillText(
      `Total p = (${totalP.x.toFixed(1)}, ${totalP.y.toFixed(1)}) kg·m/s`,
      20,
      80
    );
  ...
  function drawRocket() {
    ...
    ctx.fillText(
      `Rocket momentum = ${rocketMomentum.toFixed(1)} kg·m/s (upward)`,
      20,
      30
    );
    ctx.fillText(
      `Exhaust momentum = ${-particlesTotalMomentum.toFixed(
        1
      )} kg·m/s (downward)`,
      20,
      50
    ...
    ctx.fillText(
      "As the rocket expels mass downward, it gains upward momentum.",
      20,
      80
    );
  ...
  function drawSkaters() {
    ...
    if (scenarioObjects.skaters.hasPushed) {
      drawVector(
        skater1.x + skater1.width / 2,
        skater1.y + skater1.height / 2,
        skater1.velocity.x * 50,
        0,
        "#3498db"
      );
      drawVector(
        skater2.x + skater2.width / 2,
        skater2.y + skater2.height / 2,
        skater2.velocity.x * 50,
        0,
        "#e74c3c"
      );
    }
    ...
    ctx.fillText(
      `Skater 1 (${skater1.mass} kg): p = ${p1.toFixed(1)} kg·m/s`,
      20,
      30
    );
    ctx.fillText(
      `Skater 2 (${skater2.mass} kg): p = ${p2.toFixed(1)} kg·m/s`,
      20,
      50
    );
    ...
    if (scenarioObjects.skaters.hasPushed) {
      ctx.fillText(
        "When the skaters push off each other, they move in opposite directions.",
        20,
        110
      );
      ctx.fillText(
        "The lighter skater moves faster, but the total momentum remains zero!",
        20,
        130
      );
    } else if (animationTime > 0.5) {
      ctx.fillText(
        "Skaters are about to push off from each other...",
        width / 2 - 150,
        height / 2 - 50
      );
    }
  ...
  function drawCarCrash() {
    ...
    ctx.fillStyle = "#2c3e50";
    ctx.fillRect(
      car1.x + car1.width * 0.1,
      car1.y + car1.height,
      car1.width * 0.2,
      car1.height * 0.3
    );
    ctx.fillRect(
      car1.x + car1.width * 0.7,
      car1.y + car1.height,
      car1.width * 0.2,
      car1.height * 0.3
    );
    ...
    ctx.fillText(
      `Car 1 (${car1.mass} kg): p = ${p1.toFixed(1)} kg·m/s`,
      20,
      30
    );
    ctx.fillText(
      `Car 2 (${car2.mass} kg): p = ${p2.toFixed(1)} kg·m/s`,
      20,
      50
    );
    ctx.fillText(`Total momentum = ${totalP.toFixed(1)} kg·m/s`, 20, 80);
    ...
    if (scenarioObjects.carCrash.hasCollided) {
      ctx.fillText(
        "After collision, the total momentum remains the same!",
        20,
        110
      );
      ctx.fillText(
        "The heavier car changes velocity less than the lighter car.",
        20,
        130
      );
    }
